...
for i in range(0, len(X)):
    ...
    if(i%1000 == 0):
        print ("\033[A                             \033[A")
        print(colored("Corpus Progress: " + str(i) + "/" + str(len(X)), "green"))


print ("\033[A                             \033[A")
print(colored("Finished stemming corpus", "green"))
...
train_xs, val_xs, train_ys, val_ys = train_test_split(xs, ys, test_size=0.3)


history = model.fit(train_xs,train_ys,validation_data=(val_xs,val_ys),epochs=20,batch_size=64, verbose=2)

print(colored("Finished training", "green"))

results = model.evaluate(val_xs, val_ys)
...
pd.DataFrame.from_dict(history.history).to_csv("results/overfit.csv", index=False)